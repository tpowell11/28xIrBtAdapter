...
async def handle_ds():
    command("DS\r", ds_char) 
    
async def handle_id():
    command("ID\r", id_char)
    
async def handle_ri():
    command("RI\r", ri_char)

async def handle_rmp():
    command("RMP\r", rmp_char)

async def handle_qm():
    command("QM\r", qm_char)

async def handle_qdda():
    command("QDDA\r", qdda_char)
...
async def main():
    # setup
    is18x = Pin(4, Pin.IN, Pin.PULL_DOWN)
